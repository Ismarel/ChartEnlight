...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    open func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        drawDashedLine = dataSet.isDrawDashedLineEnabled
        ...
        var isSingleTexture = false
        if dataSet.isSingleTexture == "true"{
            isSingleTexture = true
        }
        if isSingleTexture && dataSet.isTexture == "isTexture"
        {
            let gradient:CGGradient
            gradient = dataSet.texture(atIndex: 0)
        ...
        for j in stride(from: 0, to: buffer.rects.count, by: 1)
        {
            ...
            if !isSingleTexture && dataSet.isTexture == "isTexture"
            {
                ...
                let gradient:CGGradient
                gradient = dataSet.texture(atIndex: j)
    ...
    {
        ChartUtils.drawText(context: context, text: value, point: CGPoint(x: xPos, y: yPos), align: align, attributes: [NSAttributedString.Key(rawValue: NSAttributedString.Key.font.rawValue): font, NSAttributedString.Key(rawValue: NSAttributedString.Key.foregroundColor.rawValue): color])
    }
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            ...
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? BarChartDataEntry
            {
                ...
                /*Custom Dashed line*/
                if drawDashedLine{
                    let p0 = CGPoint(x: barRect.minX, y: barRect.midY)
                    context.move(to: p0)
                    
                    let p1 = CGPoint(x: barRect.maxX, y: barRect.midY)
                    context.move(to: p1)
